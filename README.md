...
- **Ask your inbox** (RAG chat): natural-language questions over all your mail, answered by Claude with citations back to the exact emails. Embeddings run locally and free via [Ollama](https://ollama.com); retrieval is hybrid (pgvector + Postgres full-text) followed by a local cross-encoder reranker.
...
Next.js (App Router) · Drizzle ORM · Postgres 16 + pgvector (Docker) · googleapis · @typesafe-ai/sdk · @anthropic-ai/sdk · Ollama
...
Set `ANTHROPIC_API_KEY` to enable the **Ask** chat panel (answers are generated by Claude).

### 2b. Local embedding model (for chat)

The chat indexes your email with a free, open-weight embedding model running locally through Ollama:

```bash
brew install --cask ollama-binary   # or download from https://ollama.com/download
ollama serve                        # leave running
ollama pull qwen3-embedding:0.6b    # ~640 MB, one time
```

To use Voyage AI instead, set `EMBEDDING_PROVIDER=voyage` and `VOYAGE_API_KEY` (see `.env.example`).
...
`GET /api/index` reports how much of the inbox is indexed and whether the embedding model is reachable.
